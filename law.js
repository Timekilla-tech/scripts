...
function showLaw(file) {
    ...
    // Update link input
    const inp = document.getElementById('lawLinkInput');
...
function initLawPage() {
    ...
    // Clone sidebar into mobile drawer
    document.getElementById('mobileSidebarBody').innerHTML =
        document.getElementById('sidebar').innerHTML;
    ...
    // ── LINK INPUT COPY ──
    const lawLinkInput = document.getElementById('lawLinkInput');
    if (lawLinkInput) {
        lawLinkInput.addEventListener('click', function() {
            this.select();
            try {
                document.execCommand('copy');
                showToast('Холбоос хуулж авлаа');
            } catch(e) {
                navigator.clipboard && navigator.clipboard.writeText(this.value)
